...
class UnsubScribeToTopicUseCaseTest{
    ...
    private val dummyTopic = Dummy.topicNet.mapNetToGeneral()[0]

    @Test
    fun getTopicWithSubscribeStatusSuccesfully() = coroutineRule.runBlockingTest {
        val useCase = UnsubScribeToTopicUseCase(coroutineRule.testDispatcher,succesTopicRepository)
    ...
    fun getTopicWithSubscribeStatusUnSuccesfully() = coroutineRule.runBlockingTest {
        val useCase = GetTopicWithSubscribeStatusUseCase(coroutineRule.testDispatcher,
            unsuccesfulTopicRepository
        )
